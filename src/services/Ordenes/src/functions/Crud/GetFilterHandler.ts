...
export const main = authMiddleware(async (event, context) => {
  ...
  const referenceKeys = ["client","organization"]; // Explicitly define the reference keys

  for (const referenceKey of referenceKeys) {
    const RefModel = referenceKey === "client" ? Client : Organization; // Explicitly define the RefModel
    pipeline.push({
      $lookup: {
